...
@views.route('/get_latest_data', methods=['GET'])
def get_latest_data():
    latest_data = mongo.get_latest_data()
    qlty = 64
    data = {"timestamp": latest_data['DHT']['timestamp'], 
            "temperature": round(latest_data['DHT']['temperature'], 2), 
            "pressure": round(latest_data['BME']['pressure'], 2), 
            "humidity": round(latest_data['DHT']['humidity'], 2), 
            "air_quality": qlty}
...
def get_dashboard_data():
    ...
    qlty = "In Progress"
    latest_data = mongo.get_latest_data()
    latest_day_data = mongo.get_day_sensor_data(latest_data['DHT']['timestamp'].split()[0])
    latest_day_data = mongo.convert_day_chart_data(latest_day_data)
    ...
    return data, latest_day_data

@views.route('/dashboard')
def dashboard():
    data, latest_day_data = get_dashboard_data()
    return render_template('index.html', title="Eco_Health_Tracker", data=data, latest_day_data=latest_day_data)


@views.route('/get_dashboard_data', methods=['GET'])
def get_dashboard_data_json():
    data, latest_day_data = get_dashboard_data()
    return jsonify({'data': data, 'latest_day_data': latest_day_data})
